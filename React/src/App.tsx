...
import DataGrid, {
  Column, Editing, Button as ColumnButton, Toolbar, Item as GridToolbarItem, type DataGridRef,
} from 'devextreme-react/data-grid';
import Popup, { ToolbarItem } from 'devextreme-react/popup';
import Form, {
  Item, GroupItem, Label, RequiredRule, EmailRule,
...
function App(): JSX.Element {
  ...
  const dataSource = useMemo(() => ({ store: employeeStore }), []);
  ...
  const confirmChanges = useCallback(async () => {
    ...
    try {
      if (isNewRecord) {
        await employeeStore.insert(formData as Employee);
      } else if (formData.ID !== undefined) {
        await employeeStore.update(formData.ID, formData as Employee);
      }
